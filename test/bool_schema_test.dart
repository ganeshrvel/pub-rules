...
void main() {
  group('isRequired', () {
    test('null fails', () {
      final r = Rule.boolean(name: 'Terms').isRequired().parse(null);
      expect(r.hasError, isTrue);
      expect(r.error?.message, equals('Terms is required'));
    });
    test('false is present and passes required', () {
      final r = Rule.boolean(name: 'Terms').isRequired().parse(false);
      expect(r.ok, isTrue);
    });
    test('null optional passes', () {
      final r = Rule.boolean(name: 'Terms').parse(null);
      expect(r.ok, isTrue);
    });
  });
  ...
  group('check placeholder support', () {
    test('check default message uses the name placeholder', () {
      final r = Rule.boolean(name: 'Flag').check((v) => false).parse(true);
  ...
  group('isTrue / isFalse', () {
    test('isTrue fails on false', () {
      final r = Rule.boolean(name: 'Terms').isTrue().parse(false);
      expect(r.error?.message, equals('Terms must be true'));
    });
    test('isTrue passes on true', () {
      final r = Rule.boolean(name: 'Terms').isTrue().parse(true);
      expect(r.ok, isTrue);
    });
    test('isFalse fails on true', () {
      final r = Rule.boolean(name: 'Hidden').isFalse().parse(true);
      expect(r.error?.message, equals('Hidden must be false'));
    });
    test('isFalse passes on false', () {
      final r = Rule.boolean(name: 'Hidden').isFalse().parse(false);
      expect(r.ok, isTrue);
    });
  ...
  group('check and refine', () {
    ...
    test('required fires before isTrue', () {
      final r = Rule.boolean(name: 'Terms').isRequired().isTrue().parse(null);
      expect(r.error?.message, equals('Terms is required'));
  ...
  group('equalTo', () {
    test('equalTo true passes on true', () {
      final r = Rule.boolean(name: 'Terms').equalTo(true).parse(true);
      expect(r.ok, isTrue);
    });

    test('equalTo true fails on false', () {
      final r = Rule.boolean(name: 'Terms').equalTo(true).parse(false);
    ...
    test('equalTo false passes on false', () {
      final r = Rule.boolean(name: 'Hidden').equalTo(false).parse(false);
      expect(r.ok, isTrue);
    });

    test('equalTo false fails on true', () {
      final r = Rule.boolean(name: 'Hidden').equalTo(false).parse(true);
    ...
    test('equalTo is equivalent to isTrue', () {
      final viaEqualTo = Rule.boolean(name: 'Terms').equalTo(true).parse(false);
      final viaIsTrue = Rule.boolean(name: 'Terms').isTrue().parse(false);
      expect(viaEqualTo.ok, equals(viaIsTrue.ok));
    });

    test('equalTo is equivalent to isFalse', () {
      final viaEqualTo =
          Rule.boolean(name: 'Hidden').equalTo(false).parse(true);
      final viaIsFalse = Rule.boolean(name: 'Hidden').isFalse().parse(true);
      expect(viaEqualTo.ok, equals(viaIsFalse.ok));
    });

    test('required fires before equalTo', () {
      final r =
          Rule.boolean(name: 'Terms').isRequired().equalTo(true).parse(null);
      expect(r.error?.message, equals('Terms is required'));
      expect(r.error?.check, equals(BoolCheck.isRequired));
    });
