...
void main() {
  group('isRequired', () {
    test('null fails', () {
      final r = Rule.integer(name: 'Age').isRequired().parse(null);
      expect(r.hasError, isTrue);
      expect(r.error?.message, equals('Age is required'));
    });
    test('zero is present and passes required', () {
      final r = Rule.integer(name: 'Age').isRequired().parse(0);
      expect(r.ok, isTrue);
    });
    test('null optional passes', () {
      final r = Rule.integer(name: 'Age').parse(null);
      expect(r.ok, isTrue);
    });
  ...
  group('lists', () {
    test('inList membership fails', () {
      final r = Rule.integer(name: 'v').inList([0, 1, 2]).parse(10);
      expect(
        r.error?.message,
    ...
    test('inList passes', () {
      final r = Rule.integer(name: 'v').inList([1, 2]).parse(1);
      expect(r.ok, isTrue);
    });
    test('notInList membership fails', () {
      final r = Rule.integer(name: 'v').notInList([0, 1, 2]).parse(1);
      expect(
        r.error?.message,
  ...
  group('ordering', () {
    test('first failing check wins', () {
      final r = Rule.integer(name: 'v').greaterThan(10).lessThan(5).parse(3);
  ...
  group('isRequired ordering', () {
    test('isRequired fires before greaterThan', () {
      final r = Rule.integer(name: 'v').isRequired().greaterThan(0).parse(null);
  ...
  group('comparisons additional', () {
    test('lessThan passes', () {
      final r = Rule.integer(name: 'v').lessThan(5).parse(1);
      expect(r.ok, isTrue);
    });

    test('greaterThanOrEqualTo below boundary fails', () {
      final r = Rule.integer(name: 'v').greaterThanOrEqualTo(5).parse(4);
      expect(
        r.error?.message,
    ...
    test('lessThanOrEqualTo above boundary fails', () {
      final r = Rule.integer(name: 'v').lessThanOrEqualTo(0).parse(1);
      expect(r.error?.message, contains('should be less than or equal to 0'));
    });

    test('equalTo match passes', () {
      final r = Rule.integer(name: 'v').equalTo(5).parse(5);
      expect(r.ok, isTrue);
    });

    test('notEqualTo differing values passes', () {
      final r = Rule.integer(name: 'v').notEqualTo(0).parse(1);
      expect(r.ok, isTrue);
    });
  });
  ...
  group('lists additional', () {
    test('notInList membership passes', () {
      final r = Rule.integer(name: 'v').notInList([0, 1, 2]).parse(10);
      expect(r.ok, isTrue);
    });
  });
  ...
  group('per-check error overrides', () {
    test('greaterThan custom error', () {
      final r =
          Rule.integer(name: 'v').greaterThan(1, error: 'Too low').parse(1);
    ...
    test('lessThan custom error', () {
      final r = Rule.integer(name: 'v').lessThan(1, error: 'Too high').parse(1);
    ...
    test('equalTo custom error', () {
      final r =
          Rule.integer(name: 'v').equalTo(0, error: 'Must be zero').parse(1);
    ...
    test('inList custom error', () {
      final r =
          Rule.integer(name: 'v').inList([1, 2], error: 'Pick 1 or 2').parse(3);
      expect(r.error?.message, equals('Pick 1 or 2'));
    });

    test('notInList custom error', () {
      final r =
          Rule.integer(name: 'v').notInList([0], error: 'Cannot be 0').parse(0);
  ...
  group('check and refine additional', () {
    test('check fails with default message', () {
      final r = Rule.integer(name: 'v').check((v) => v > 2).parse(1);
      expect(r.error?.message, equals('v is invalid'));
    });

    test('check passes when predicate is true', () {
      final r = Rule.integer(name: 'v').check((v) => v.isEven).parse(4);
      expect(r.ok, isTrue);
    });
  ...
  group('ordering additional', () {
    test('isRequired fires before lessThan', () {
      final r = Rule.integer(name: 'v').isRequired().lessThan(5).parse(null);
      expect(r.error?.check, equals(IntCheck.isRequired));
    });
  ...
  group('validatedValue and hasValidatedValue', () {
    ...
    test('failed validation, hasValidatedValue false', () {
      final r = Rule.integer(name: 'v').greaterThan(10).parse(1);
      expect(r.hasError, isTrue);
      expect(r.hasValidatedValue, isFalse);
      expect(r.validatedValue, isNull);
    });
