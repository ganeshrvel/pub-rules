...
void main() {
  group('isRequired', () {
    test('null fails', () {
      final r = Rule.string(name: 'Name').isRequired().validate(null);
    ...
    test('empty string fails', () {
      final r = Rule.string(name: 'Name').isRequired().validate('');
      expect(r.hasError, isTrue);
      expect(r.error?.message, equals('Name is required'));
    });
    test('present value passes', () {
      final r = Rule.string(name: 'Name').isRequired().validate('abc');
      expect(r.ok, isTrue);
      expect(r.validatedValue, equals('abc'));
    });
    test('null optional passes', () {
      final r = Rule.string(name: 'Name').validate(null);
      expect(r.ok, isTrue);
    });
    test('empty optional passes', () {
      final r = Rule.string(name: 'Name').validate('');
      expect(r.ok, isTrue);
    });
  ...
  group('isEmail', () {
    test('valid email passes', () {
      final r = Rule.string(name: 'Email').isEmail().validate('abc@xyz.com');
      expect(r.ok, isTrue);
    });
    test('missing tld fails', () {
      final r = Rule.string(name: 'Email').isEmail().validate('abc@xyz');
  ...
  group('isUrl', () {
    test('protocol url passes', () {
      final r =
          Rule.string(name: 'value').isUrl().validate('http://www.google.com');
      expect(r.ok, isTrue);
    });
    test('bare domain passes', () {
      final r = Rule.string(name: 'value').isUrl().validate('www.google.com');
      expect(r.ok, isTrue);
    });
    test('localhost without protocol fails', () {
      final r = Rule.string(name: 'value').isUrl().validate('localhost');
      expect(r.error?.message, contains('is not a valid URL'));
    });
    test('plain text fails', () {
      final r = Rule.string(name: 'value').isUrl().validate('not a url');
      expect(r.hasError, isTrue);
    });
  });
  ...
  group('isPhone', () {
    test('valid phone passes', () {
      final r = Rule.string(name: 'value').isPhone().validate('+1-9090909090');
      expect(r.ok, isTrue);
    });
    test('non-phone fails', () {
      final r = Rule.string(name: 'value').isPhone().validate('abc');
  ...
  group('isIp', () {
    test('valid ipv4 passes', () {
      final r = Rule.string(name: 'value').isIp().validate('1.1.1.1');
      expect(r.ok, isTrue);
    });
    test('non-ip fails', () {
      final r = Rule.string(name: 'value').isIp().validate('abc');
  ...
  group('isNumeric', () {
    test('positive integer passes', () {
      final r = Rule.string(name: 'value').isNumeric().validate('1');
      expect(r.ok, isTrue);
    });
    test('zero passes', () {
      final r = Rule.string(name: 'value').isNumeric().validate('0');
      expect(r.ok, isTrue);
    });
    ...
    test('decimal fails', () {
      final r = Rule.string(name: 'value').isNumeric().validate('1.5');
      expect(r.error?.message, contains('is not a valid number'));
    });
    test('alpha fails', () {
      final r = Rule.string(name: 'value').isNumeric().validate('abc');
      expect(r.hasError, isTrue);
    });
    test('custom error override', () {
  ...
  group('isNumericDecimal', () {
    test('decimal passes', () {
      final r = Rule.string(name: 'value').isNumericDecimal().validate('10.01');
      expect(r.ok, isTrue);
    });
    test('negative decimal passes', () {
      final r =
          Rule.string(name: 'value').isNumericDecimal().validate('-10.01');
      expect(r.ok, isTrue);
    });
    test('leading zero decimal passes', () {
      final r = Rule.string(name: 'value').isNumericDecimal().validate('0.001');
      expect(r.ok, isTrue);
    });
    test('whole number passes', () {
      final r = Rule.string(name: 'value').isNumericDecimal().validate('5');
      expect(r.ok, isTrue);
    });
    test('alpha fails', () {
      final r = Rule.string(name: 'value').isNumericDecimal().validate('abc');
  ...
  group('isAlphaSpace', () {
    test('letters and spaces pass', () {
      final r = Rule.string(name: 'value').isAlphaSpace().validate('Jane Doe');
      expect(r.ok, isTrue);
    });
    test('digits fail', () {
      final r = Rule.string(name: 'value').isAlphaSpace().validate('abc123');
      expect(
        r.error?.message,
  ...
  group('isAlphaNumeric', () {
    test('letters and digits pass', () {
      final r =
          Rule.string(name: 'value').isAlphaNumeric().validate('username123');
      expect(r.ok, isTrue);
    });
    test('space fails', () {
      final r =
          Rule.string(name: 'value').isAlphaNumeric().validate('user name');
      expect(
        r.error?.message,
  ...
  group('isAlphaNumericSpace', () {
    test('letters digits and spaces pass', () {
      final r =
          Rule.string(name: 'value').isAlphaNumericSpace().validate('Bread 20');
      expect(r.ok, isTrue);
    });
    test('symbol fails', () {
      final r =
  ...
  group('matches', () {
    test('matching pattern passes', () {
      final r =
          Rule.string(name: 'value').regex(RegExp(r'^\d+$')).validate('123');
      expect(r.ok, isTrue);
    });
    test('non-matching pattern fails', () {
      final r =
          Rule.string(name: 'value').regex(RegExp(r'^\d+$')).validate('abc');
  ...
  group('length constraints', () {
    test('exact length passes', () {
      final r = Rule.string(name: 'value').length(3).validate('abc');
      expect(r.ok, isTrue);
    });
    test('wrong length fails', () {
      final r = Rule.string(name: 'value').length(3).validate('ab');
      expect(r.error?.message, equals('value should be 3 characters long'));
    });
    test('minLength boundary passes', () {
      final r = Rule.string(name: 'value').minLength(3).validate('abc');
      expect(r.ok, isTrue);
    });
    test('below minLength fails', () {
      final r = Rule.string(name: 'value').minLength(3).validate('ab');
      expect(
        r.error?.message,
    ...
    test('maxLength boundary passes', () {
      final r = Rule.string(name: 'value').maxLength(3).validate('abc');
      expect(r.ok, isTrue);
    });
    test('above maxLength fails', () {
      final r = Rule.string(name: 'value').maxLength(3).validate('abcd');
      expect(
        r.error?.message,
  ...
  group('comparisons', () {
    test('equalTo mismatch fails', () {
      final r = Rule.string(name: 'value').equalTo('abc').validate('xyz');
      expect(r.error?.message, equals('value should be equal to abc'));
    });
    test('notEqualTo match fails', () {
      final r = Rule.string(name: 'value').notEqualTo('abc').validate('abc');
  ...
  group('numeric comparisons on strings', () {
    ...
    test('lessThan handles negative decimal numbers correctly', () {
      final r = Rule.string(name: 'v').lessThan(-1.5).validate('-2.5');
      expect(r.ok, isTrue);
    });
  });
  ...
  group('numeric comparisons reject non-numeric input', () {
    test('greaterThan fails when the value is not numeric', () {
      final r = Rule.string(name: 'v').greaterThan(5).validate('abc');
      expect(r.error?.message, equals('v is not a valid number'));
    });

    test('lessThan fails when the value is not numeric', () {
      final r = Rule.string(name: 'v').lessThan(5).validate('abc');
      expect(r.error?.message, equals('v is not a valid number'));
    });

    test('greaterThanOrEqualTo fails when the value is not numeric', () {
      final r = Rule.string(name: 'v').greaterThanOrEqualTo(5).validate('abc');
      expect(r.error?.message, equals('v is not a valid number'));
    });

    test('lessThanOrEqualTo fails when the value is not numeric', () {
      final r = Rule.string(name: 'v').lessThanOrEqualTo(5).validate('abc');
      expect(r.error?.message, equals('v is not a valid number'));
    });
    ...
    test('custom error also overrides the failed-comparison message', () {
      final r =
          Rule.string(name: 'v').lessThan(5, error: 'Too big').validate('10');
  ...
  group('lists', () {
    test('inList membership passes', () {
      final r = Rule.string(name: 'value').inList(['a', 'b']).validate('a');
      expect(r.ok, isTrue);
    });
    test('inList membership fails', () {
      final r = Rule.string(name: 'value').inList(['a', 'b']).validate('c');
      expect(
        r.error?.message,
    ...
    test('notInList membership passes', () {
      final r = Rule.string(name: 'value').notInList(['a', 'b']).validate('c');
      expect(r.ok, isTrue);
    });
    test('notInList membership fails', () {
      final r = Rule.string(name: 'value').notInList(['a', 'b']).validate('a');
      expect(
        r.error?.message,
  ...
  group('shouldMatch / shouldNotMatch', () {
    test('shouldMatch passes', () {
      final r = Rule.string(name: 'value').shouldMatch('abc').validate('abc');
      expect(r.ok, isTrue);
    });
    test('shouldMatch fails', () {
      final r = Rule.string(name: 'value').shouldMatch('abc').validate('xyz');
      expect(r.error?.message, equals('value should be same as abc'));
    });
    test('shouldNotMatch passes', () {
      final r =
          Rule.string(name: 'value').shouldNotMatch('xyz').validate('abc');
      expect(r.ok, isTrue);
    });
    test('shouldNotMatch fails', () {
      final r =
          Rule.string(name: 'value').shouldNotMatch('xyz').validate('xyz');
  ...
  group('transforms', () {
    test('untrimmed email fails', () {
      final r =
          Rule.string(name: 'email').isEmail().validate('  abc@xyz.com  ');
      expect(r.hasError, isTrue);
    });
    test('trim before email passes', () {
      final r = Rule.string(name: 'email')
          .trim()
          .isEmail()
          .validate('  abc@xyz.com  ');
      expect(r.ok, isTrue);
    });
    test('trim to empty fails required', () {
      final r = Rule.string(name: 'name').isRequired().trim().validate('  ');
      expect(r.hasError, isTrue);
    });
    test('trim to empty passes optional', () {
      final r = Rule.string(name: 'name').trim().validate('  ');
      expect(r.ok, isTrue);
    });
    test('lowercase value has no uppercase match', () {
      final r =
          Rule.string(name: 'Name').regex(RegExp('[A-Z]')).validate('abc');
      expect(r.hasError, isTrue);
    });
  ...
  group('check', () {
    test('predicate passes', () {
      final r =
          Rule.string(name: 'value').check((v) => v.length > 2).validate('abc');
      expect(r.ok, isTrue);
    });
    test('predicate fails with default message', () {
      final r =
          Rule.string(name: 'value').check((v) => v.length > 2).validate('a');
  ...
  group('refine', () {
    ...
    test('null return passes', () {
      final r =
          Rule.string(name: 'value').refine((v) => null).validate('qwerty123');
      expect(r.ok, isTrue);
    });
  ...
  group('ordering and short-circuit', () {
    ...
    test('first failing check wins', () {
      final r =
          Rule.string(name: 'value').isAlphaSpace().isEmail().validate('123');
      expect(
        r.error?.message,
        equals('Only alphabets and spaces are allowed in value'),
      );
    ...
    test('checks skipped for absent optional value', () {
      final r = Rule.string(name: 'value').isEmail().validate('');
      expect(r.ok, isTrue);
  ...
  group('trim interactions', () {
    test('trim + isRequired, value with only spaces fails', () {
      final r = Rule.string(name: 'Name').trim().isRequired().validate('   ');
      expect(r.hasError, isTrue);
      expect(r.error?.message, equals('Name is required'));
    });
    ...
    test('trim + isRequired, value with spaces around text passes', () {
      final r =
          Rule.string(name: 'Name').trim().isRequired().validate('  john  ');
      expect(r.ok, isTrue);
    ...
    test('trim + shouldMatch, trimmed value matches passes', () {
      final r =
          Rule.string(name: 'v').trim().shouldMatch('abc').validate('  abc  ');
      expect(r.ok, isTrue);
    });

    test('trim + shouldMatch, trimmed value does not match fails', () {
      final r =
          Rule.string(name: 'v').trim().shouldMatch('abc').validate('  xyz  ');
      expect(r.hasError, isTrue);
    });
  ...
  group('trim + toLowerCase + toUpperCase combined', () {
    ...
    test('trim + toUpperCase together', () {
      final r =
          Rule.string(name: 'Name').trim().toUpperCase().validate('  john  ');
      expect(r.ok, isTrue);
      expect(r.validatedValue, equals('JOHN'));
    });
  ...
  group('refine placeholder combinations', () {
    ...
    test('value placeholder resolved', () {
      final r =
          Rule.string(name: 'v').refine((v) => 'got {value}').validate('hello');
  ...
  group('isRequired ordering with other checks', () {
    test('isRequired fires before isEmail', () {
      final r = Rule.string(name: 'Email').isRequired().isEmail().validate('');
      expect(r.error?.check, equals(StringCheck.isRequired));
    });

    test('isRequired fires before isNumeric', () {
      final r = Rule.string(name: 'v').isRequired().isNumeric().validate('');
      expect(r.error?.check, equals(StringCheck.isRequired));
    });

    test('isRequired fires before minLength', () {
      final r = Rule.string(name: 'v').isRequired().minLength(3).validate('');
      expect(r.error?.check, equals(StringCheck.isRequired));
    });
  ...
  group('per-check error overrides', () {
    ...
    test('isUrl custom error', () {
      final r =
          Rule.string(name: 'v').isUrl(error: 'Bad URL').validate('notaurl');
      expect(r.error?.message, equals('Bad URL'));
    });

    test('isPhone custom error', () {
      final r =
          Rule.string(name: 'v').isPhone(error: 'Bad phone').validate('abc');
      expect(r.error?.message, equals('Bad phone'));
    });

    test('isIp custom error', () {
      final r = Rule.string(name: 'v').isIp(error: 'Bad IP').validate('abc');
    ...
    test('length custom error', () {
      final r =
          Rule.string(name: 'v').length(4, error: 'Must be 4').validate('abc');
    ...
    test('inList custom error', () {
      final r = Rule.string(name: 'v')
          .inList(['a', 'b'], error: 'Pick a or b').validate('c');
      expect(r.error?.message, equals('Pick a or b'));
    });

    test('notInList custom error', () {
      final r = Rule.string(name: 'v')
          .notInList(['a', 'b'], error: 'Cannot be a or b').validate('a');
  ...
  group('validatedValue and hasValidatedValue across types', () {
    ...
    test('string null optional, hasValidatedValue false', () {
      final r = Rule.string(name: 'v').validate(null);
      expect(r.ok, isTrue);
      expect(r.hasValidatedValue, isFalse);
      expect(r.validatedValue, isNull);
    });
    ...
    test('string failed validation, hasValidatedValue false', () {
      final r = Rule.string(name: 'v').isEmail().validate('notanemail');
      expect(r.hasError, isTrue);
      expect(r.hasValidatedValue, isFalse);
      expect(r.validatedValue, isNull);
    });
    ...
    test('string after trim, validatedValue is trimmed', () {
      final r = Rule.string(name: 'v').trim().validate('  hello  ');
      expect(r.hasValidatedValue, isTrue);
      expect(r.validatedValue, equals('hello'));
    });
  ...
  group('fold result', () {
    test('onValidatedValue promoted for present valid string', () {
      final promoted =
          Rule.string(name: 'v').isRequired().validate('hello').fold(
    ...
    test('onNull promoted for empty optional string', () {
      final nullFired = Rule.string(name: 'v').validate('').fold(
            onOk: (ok, {required name}) => ok.fold(
    ...
    test('onNull promoted for null optional integer', () {
      final nullFired = Rule.integer(name: 'Age').validate(null).fold(
            onOk: (ok, {required name}) => ok.fold(
              onValidatedValue: ({required value}) => false,
              onNull: () => true,
            ),
    ...
    test('onValidatedValue promoted for integer zero', () {
      final promoted = Rule.integer(name: 'Score').validate(0).fold(
            onOk: (ok, {required name}) => ok.fold(
              onValidatedValue: ({required value}) => value,
              onNull: () => null,
            ),
    ...
    test('onError promoted for failed required string', () {
      final errorMessage =
          Rule.string(name: 'v').isRequired().validate('').fold(
                onOk: (ok, {required name}) => ok.fold(
    ...
    test('onError promoted for failed isEmail', () {
      final errorMessage =
          Rule.string(name: 'Email').isEmail().validate('notanemail').fold(
                onOk: (ok, {required name}) => ok.fold(
                  onValidatedValue: ({required value}) => null,
                  onNull: () => null,
                ),
    ...
    test('name is passed correctly into onOk', () {
      final foldedName = Rule.string(name: 'MyField').validate('hello').fold(
    ...
    test('name is passed correctly into onError', () {
      final foldedName =
          Rule.string(name: 'MyField').isRequired().validate('').fold(
                onOk: (ok, {required name}) => name,
                onError: ({required name, required error}) => name,
              );
      expect(foldedName, equals('MyField'));
    ...
    test('onValidatedValue assigns the raw value for required and valid email',
        () {
      final display =
          Rule.string(name: 'Email').isRequired().isEmail().validate('').fold(
                onOk: (ok, {required name}) => ok.fold(
                  onValidatedValue: ({required value}) => value,
                  onNull: () => '',
                ),
    ...
    test('onValidatedValue assigns the value length for a valid email', () {
      final length = Rule.string(name: 'Email')
          .isEmail()
          .validate('user@example.com')
          .fold(
            onOk: (ok, {required name}) => ok.fold(
    ...
    test('onValidatedValue assigns true for a valid email', () {
      final isValid = Rule.string(name: 'Email')
          .isEmail()
          .validate('user@example.com')
          .fold(
            onOk: (ok, {required name}) => ok.fold(
    ...
    test('onNull assigns empty string for a null optional phone', () {
      final display = Rule.string(name: 'Phone').validate(null).fold(
            onOk: (ok, {required name}) => ok.fold(
              onValidatedValue: ({required value}) => value,
              onNull: () => '',
            ),
    ...
    test('onNull assigns a name-prefixed message for an empty optional phone',
        () {
      final display = Rule.string(name: 'Phone').validate('').fold(
            onOk: (ok, {required name}) => ok.fold(
              onValidatedValue: ({required value}) => value,
    ...
    test('onNull assigns zero length for an empty optional phone', () {
      final length = Rule.string(name: 'Phone').validate('').fold(
            onOk: (ok, {required name}) => ok.fold(
              onValidatedValue: ({required value}) => value.length,
              onNull: () => 0,
            ),
    ...
    test('onNull assigns false for a null optional phone', () {
      final isValid = Rule.string(name: 'Phone').validate(null).fold(
            onOk: (ok, {required name}) => ok.fold(
              onValidatedValue: ({required value}) => true,
              onNull: () => false,
            ),
    ...
    test('fold onNull promoted after trim collapses to empty', () {
      final nullFired = Rule.string(name: 'v').trim().validate('   ').fold(
            onOk: (ok, {required name}) => ok.fold(
              onValidatedValue: ({required value}) => false,
              onNull: () => true,
            ),
    ...
    test('fold onValidatedValue for bool false', () {
      final promoted = Rule.boolean(name: 'Newsletter').validate(false).fold(
            onOk: (ok, {required name}) => ok.fold(
              onValidatedValue: ({required value}) => value,
              onNull: () => null,
            ),
    ...
    test('fold onNull for bool null optional', () {
      final nullFired = Rule.boolean(name: 'Newsletter').validate(null).fold(
            onOk: (ok, {required name}) => ok.fold(
              onValidatedValue: ({required value}) => false,
              onNull: () => true,
            ),
