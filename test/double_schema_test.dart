...
void main() {
  group('isRequired', () {
    test('null fails', () {
      final r = Rule.double(name: 'Price').isRequired().parse(null);
      expect(r.hasError, isTrue);
    });
    test('0.0 is present and passes', () {
      final r = Rule.double(name: 'Price').isRequired().parse(0.0);
      expect(r.ok, isTrue);
    });
    test('null optional passes', () {
      final r = Rule.double(name: 'Price').parse(null);
      expect(r.ok, isTrue);
    });
  });
  ...
  group('isInteger', () {
    test('whole-valued double passes', () {
      final r = Rule.double(name: 'v').isInteger().parse(5.0);
      expect(r.ok, isTrue);
    });
    test('fractional double fails', () {
      final r = Rule.double(name: 'v').isInteger().parse(5.5);
  ...
  group('comparisons', () {
    test('greaterThan boundary fails', () {
      final r = Rule.double(name: 'v').greaterThan(1.0).parse(1.0);
      expect(r.error?.message, contains('should be greater than 1'));
    });
    test('greaterThanOrEqualTo boundary passes', () {
      final r = Rule.double(name: 'v').greaterThanOrEqualTo(5.0).parse(5.0);
      expect(r.ok, isTrue);
    });
    test('lessThanOrEqualTo boundary passes', () {
      final r = Rule.double(name: 'v').lessThanOrEqualTo(2.0).parse(2.0);
      expect(r.ok, isTrue);
    });
    test('equalTo treats 10.0 and 10 as equal', () {
      final r = Rule.double(name: 'v').equalTo(10.0).parse(10.0);
      expect(r.ok, isTrue);
    });
    test('notEqualTo match fails', () {
      final r = Rule.double(name: 'v').notEqualTo(10.0).parse(10.0);
      expect(r.error?.message, contains('should not be equal to 10'));
    });
    test('negative comparison passes', () {
      final r = Rule.double(name: 'v').lessThanOrEqualTo(-2.0).parse(-10.0);
      expect(r.ok, isTrue);
    });
  });
  ...
  group('lists', () {
    test('inList membership passes', () {
      final r = Rule.double(name: 'v').inList([1.0, 2.0]).parse(1.0);
      expect(r.ok, isTrue);
    });
    test('notInList membership fails', () {
      final r = Rule.double(name: 'v').notInList([-10.0]).parse(-10.0);
      expect(r.hasError, isTrue);
    });
  });
  ...
  group('check and refine', () {
    ...
    test('check passes', () {
      final r = Rule.double(name: 'v').check((v) => v > 0).parse(1.0);
      expect(r.ok, isTrue);
    });
  });
  ...
  group('isRequired custom messages and ordering', () {
    ...
    test('isRequired fires before isInteger', () {
      final r = Rule.double(name: 'v').isRequired().isInteger().parse(null);
      expect(r.error?.check, equals(DoubleCheck.isRequired));
    });

    test('isRequired fires before greaterThan', () {
      final r =
          Rule.double(name: 'v').isRequired().greaterThan(0.0).parse(null);
      expect(r.error?.check, equals(DoubleCheck.isRequired));
    });
  ...
  group('comparisons additional', () {
    test('lessThan passes', () {
      final r = Rule.double(name: 'v').lessThan(5.0).parse(1.0);
      expect(r.ok, isTrue);
    });

    test('lessThan boundary fails', () {
      final r = Rule.double(name: 'v').lessThan(1.0).parse(1.0);
      expect(r.error?.message, contains('should be less than 1'));
    });

    test('greaterThanOrEqualTo below boundary fails', () {
      final r = Rule.double(name: 'v').greaterThanOrEqualTo(5.0).parse(4.9);
      expect(
          r.error?.message, contains('should be greater than or equal to 5'),);
    });

    test('lessThanOrEqualTo above boundary fails', () {
      final r = Rule.double(name: 'v').lessThanOrEqualTo(2.0).parse(2.1);
      expect(r.error?.message, contains('should be less than or equal to 2'));
    });

    test('equalTo mismatch fails', () {
      final r = Rule.double(name: 'v').equalTo(10.0).parse(9.99);
      expect(r.error?.message, contains('should be equal to 10'));
    });

    test('notEqualTo differing values passes', () {
      final r = Rule.double(name: 'v').notEqualTo(10.0).parse(9.99);
      expect(r.ok, isTrue);
    });
  });
  ...
  group('lists additional', () {
    test('inList membership fails', () {
      final r = Rule.double(name: 'v').inList([1.0, 2.0]).parse(3.0);
      expect(
        r.error?.message,
    ...
    test('notInList membership passes', () {
      final r = Rule.double(name: 'v').notInList([-10.0]).parse(5.0);
      expect(r.ok, isTrue);
    });
  });
  ...
  group('per-check error overrides', () {
    ...
    test('greaterThan custom error', () {
      final r =
          Rule.double(name: 'v').greaterThan(1.0, error: 'Too low').parse(1.0);
    ...
    test('lessThan custom error', () {
      final r =
          Rule.double(name: 'v').lessThan(1.0, error: 'Too high').parse(1.0);
    ...
    test('equalTo custom error', () {
      final r =
          Rule.double(name: 'v').equalTo(10.0, error: 'Must be 10').parse(9.0);
    ...
    test('inList custom error', () {
      final r = Rule.double(name: 'v')
          .inList([1.0, 2.0], error: 'Pick 1.0 or 2.0').parse(3.0);
      expect(r.error?.message, equals('Pick 1.0 or 2.0'));
    });

    test('notInList custom error', () {
      final r = Rule.double(name: 'v')
          .notInList([-10.0], error: 'Cannot be -10').parse(-10.0);
      expect(r.error?.message, equals('Cannot be -10'));
    });
  ...
  group('check and refine additional', () {
    test('check fails with default message', () {
      final r = Rule.double(name: 'v').check((v) => v > 2).parse(1.0);
